...
## 绘制所有Sheet中的某一列到Graph里

```labtalk
string strBook$ = %H; // 你的工作簿名称
win -a %(strBook$); // 确保它处于激活状态

int nSheets = page.nlayers; // 获取该 Book 中总共有多少个 Sheet
...
themeApply2g theme:="ShLiu_Frame";
label -p 10 90 -n myNote "This is My Notes";
...
## Comments in Books (multiple WSheets)
...
int nCol = 11;             // 你要设置注释的列号（比如第 11 列）
...
for(int ii = 1; ii <= nSheets; ii++) {
    ...
    if (nCol <= wks.ncols) {
        ...
        // 如果你想把该 Sheet 的第一列（通常是X）也改了，可以取消下面这行的注释：
        // wcol(1)[C]$ = strSheet$;
        
        type "第 $(ii) 个 Sheet: [%(strSheet$)] -> 已写入第 $(nCol) 列注释";
    }
    else {
        type "第 $(ii) 个 Sheet: [%(strSheet$)] -> 错误：总列数不足 $(nCol) 列，跳过";
    }
}
